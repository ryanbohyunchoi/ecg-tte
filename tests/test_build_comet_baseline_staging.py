...
class StagingTests(unittest.TestCase):
    ...
    def test_full_assembly_recorded_zero_and_linkage(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(s['covariates'],32);self.assertEqual(s['version'],'comet_baseline_staging_v3')
            self.assertTrue(s['counts_valid']);self.assertFalse(s['ready_for_mice'])
            t=pq.read_table(root/'out'/'restricted_baseline_staging.parquet');self.assertEqual(t.num_columns,35);self.assertEqual(t.num_rows,2)
            ...
            self.assertEqual(a['hospital_admissions'],1);self.assertNotIn('hf_hospital_admissions',a)
