...
class StagingTests(unittest.TestCase):
    ...
    def build(self,root,name,tables):
        ...
        return out
    def test_full_assembly_exact_columns_and_no_invented_negative(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp)
            dx=pa.table({'PAT_MRN_ID':['A','A'],'DX_DATE':['2023-12-01','2024-01-02'],'CURRENT_ICD10_LIST':['I50.2;I48.91','E11.9'],'PAT_ENC_CSN_ID':['E1','F']})
            core=self.build(root,'core',{
                'echo_studies':(pa.table({'PAT_MRN_ID':['A'],'EchoDate':['2023-12-01'],'EF':[30.]}),['EchoDate']),
                'hospital_diagnoses':(dx,['DX_DATE']),'outpatient_diagnoses':(dx,['DX_DATE']),
            ...
            self.assertEqual(a['atrial_fibrillation'],1);self.assertIsNone(a['diabetes'])
            self.assertEqual(a['ace_inhibitor_order'],1);self.assertIsNone(a['digoxin_order'])
