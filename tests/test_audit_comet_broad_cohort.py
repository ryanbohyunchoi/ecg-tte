...
class BroadTests(unittest.TestCase):
    ...
    def test_general_code_or_low_ef_and_no_ties(self):
        ...
        self.assertTrue(A.selected(r,'gt1_lt40'));self.assertFalse(A.selected(r,'eq40'))
        r['DX_DATE_prior_hf_evidence']=True
        self.assertTrue(A.selected(r,'missing'))
        r['candidate_arm']='competing_arms_same_day';self.assertFalse(A.selected(r,'gt1_lt40'))
    def test_end_to_end_retains_history_without_excluding_and_no_fallback(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pq.write_table(pa.table({'patient_key':['A','B','C'],'candidate_arm':['carvedilol_candidate']*3,
                ...
                'prior_365d_family_order_records':[3,0,0],'undated_family_order_records':[2,0,0]}),candidate)
            ...
            self.assertEqual(r['selected_candidate_keys'],2)
            self.assertEqual(sum(x['patient_keys'] for x in r['history_groups']),2)
            self.assertEqual(sum(x['patient_keys'] for x in r['calendar_history_groups']),2)
            self.assertEqual(sum(x['patient_keys'] for x in r['evidence_groups']),2)
            rows=pq.read_table(root/'out'/'restricted_broad_candidates.parquet').to_pylist()
            self.assertEqual({x['patient_key'] for x in rows},{'A','C'})
