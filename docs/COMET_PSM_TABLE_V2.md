# Adapted COMET baseline table — declared 32-variable target

2026-09-21. This specifies the extraction target in response to the user's request for a clean PSM table. It supersedes the open-ended feature inventory for primary extraction. It is not evidence that all32 variables are available, validated or sufficient to control confounding. Actual model readiness, transformations and missing-data rules remain gated on source review. No feature is dropped automatically to make a model run.
...
- Utilization counts require validated encounter setting and cross-delivery deduplication. The all-cause count uses unique patient/encounter keys flagged INP_YN=1 in days1–365 before index. It does not require an HF diagnosis. HF-specific hospitalization is removed from baseline adjustment by user decision; study endpoints are unchanged. Encounter counts are not yet deduplicated hospitalization episodes.
- Select creatinine as the primary renal numeric measure. eGFR is an auxiliary pending equation/source validation, not a33rd predictor. BNP and NT-proBNP remain separate optional auxiliaries/sensitivity variables; do not merge assays or add them silently. Site, race/ethnicity and other social/contextual variables remain explicit role-review candidates, not assumed irrelevant.
...
Staging v3 implements this32-variable target with three metadata columns. Prior33-variable reports remain historical. MICE and PSM have not run; labs, measurement contracts and remaining source review are still pending.
