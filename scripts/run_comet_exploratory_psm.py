...
def run(report,output,rscript):
    ...
    summary=dict(version='comet_exploratory_psm_v1',status='running',counts_valid=False,ready_for_effects=False,restricted_until_reviewed=True)
    ...
    try:
        ...
        with (output/'restricted_psm_engine.log').open('w') as log:
            proc=subprocess.run([rscript,'--vanilla',str(Path(__file__).with_name('comet_exploratory_psm.R')),str(report),str(output)],env=env,stdout=log,stderr=log)
        ...
        summary.update(status='complete_exploratory_design_requires_review',counts_valid=True,rows=source['rows'],denominators=source['denominators'],source_report=str(report),
                       ...
                       specification='32 clinical covariates, main-effects unpenalized logistic PS; sex dummy coded. Original missingness evaluated, not fitted. Carvedilol treated; 1:1 greedy descending treated logit, no replacement, caliper0.2 pooled within-arm pre-match SD(logit). SMD denominator fixed pre-match within each imputation. No effect estimation.')
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--report',required=True,type=Path);p.add_argument('--output-dir',required=True,type=Path);p.add_argument('--rscript',default='Rscript');a=p.parse_args()
    try:run(a.report,a.output_dir,a.rscript)
