...
def run(root, relative, output, patient_column='PAT_MRN_ID', max_rows=100000,
        encoding='utf-8-sig', category_limit=1000):
    ...
    try:
        ...
        if header.get('status') != 'header_candidate':
            raise ValueError('invalid_header_or_source')
        names = [name.upper() for name in header['columns']]
        if patient_column not in names:
            raise ValueError('patient_key_absent')
        ...
        if connection is not None:
            connection.close()
        # No partial counts or exception text: either may mislead or expose source data.
