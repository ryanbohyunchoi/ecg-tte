...
def choose(rows):
    ...
    return [r for k in sorted(groups) for r in sorted(groups[k],key=lambda r:r['fileID'])[:8]]
...
def run(report,out):
    ...
    summary=dict(version='comet_bcl_smoke_inputs_v1',status='complete_smoke_inputs',ready_for_inference=False,restricted_until_reviewed=True,rows=len(sample),groups=[dict(arm=a,format_250hz=b,rows=n) for (a,b),n in sorted(Counter((r['treatment_arm'],r['format_250hz']) for r in sample).items())],interpretation='Up to8 per arm/sampling stratum. Derived catalog reproduces selection flags using canonical relative stems. No waveform contents or checkpoint loaded; checkpoint/runtime review remains pending.')
