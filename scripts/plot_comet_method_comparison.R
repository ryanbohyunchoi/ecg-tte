...
write.csv(summary,file.path(root,'comparison_metrics.csv'),row.names=FALSE)
pdf(file.path(root,'comparison_love_plots.pdf'),width=10,height=13)
for(i in sort(unique(d$imputation))) {
 ...
 par(mar=c(4,15,4,1));plot(abs(base$smd_pre[ord]),seq_along(features),xlim=c(0,max(c(.1,abs(x$smd_pre),abs(x$smd_post)),na.rm=TRUE)),yaxt='n',ylab='',xlab='Absolute SMD; fixed common-population denominator',pch=1,main=paste('Saved imputation',i,'— balance and retention must be reviewed together'))
 ...
 legend('topright',c('Unadjusted','Original PSM','Previously refined PSM','CLMBR cosine'),pch=c(1,16,17,18),col=c('black',colors),cex=.7,bg='white')
}
dev.off()
